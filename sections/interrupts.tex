...
\rev{}{especially in the last-mile or outdated home cable modem boxes.
Until this point, we have considered both network consumption and 
performance metrics.} In this section, we aim to understand how VCAs respond to the types of disruptions that home Internet users might sometimes experience
\rev{}{by studying only network consumption. It is clear 
that network consumption is not a proxy for performance or QoE. However, if
...
\begin{figure}[]
   \centering
    ...
    \caption{Setup for competition experiments. When C1 is streaming Netflix or Youtube, it follows path (1).
    If C1 is a VCA, it follows path (b). When C1 run iPerf3, it follows path (c).}
