\begin{figure}[th]
    ...
    upper trendline is the incumbent call). Note that the competing client's bitrate
...
\end{subfigure}
\caption{Upstream bitrate of VCAs in competition on a 0.5~Mbps capacity link. Upper trendline is incumbent application and lower is competing application. The competing client's bitrate drops to 0 Mbps because the connection is 
terminated.}
...
\end{subfigure}
\caption{Comparison of Teams competition behavior for uplink and downlink on a 1~Mbps symmetric link. Teams is the incumbent application. In ~\ref{fig:teams-zoom-up-1}, the Zoom bitrate goes to 0 Mbps because the connection is 
terminated}
...
nominal bitrates of each VCAs. \rev{}{We include figures in Section A of the appendix 
that demonstrate that most VCAs can achieve their nominal uplink usage on the minimum 
...
Teams is extremely passive when competing against TCP. \rev{}{Note that the competing client's bitrate
drops to 0 because the connection is terminated.}
...
We found in Section 3 that Teams has a nominal uplink usage of 1.4 Mbps, whereas
here the competing client uses only 1 Mbps for the majority of the call. The FCC
...
\rev{}{The difference between how an aggressive VCA and one that backs off in the 
presence of a competing application is most evident in Figure~\ref{fig:comp_netflix_zoom_teams}. 
Zoom does not change its utilization while Teams relinquishes nearly the entire 
available bandwidth. Teams operates at extremely low utilization for nearly 75~seconds
before returning to its nominal utilization rate. Because Teams reduces utilization, 
Netflix is able to use the available bandwidth to fill a buffer, before switching to
a steady-state on-off regime. Although Netflix takes longer to fill its buffer because 
Zoom does not back off, it appears to also enter this on-off regime when competing 
with Zoom as well. In the likely event that this low utilization results in worse 
performance for Teams, it would be preferable for Teams to not back off.}
