...
that demonstrate that most VCAs, except two competing Teams clients, can achieve their nominal uplink usage on the minimum 
...
utilization is affected by a competing Netflix stream.  When the Netflix stream is initiated, it uses the available bandwidth to fill its buffer before switching to a 
steady-state on-off regime. Although Teams's utilization increases when Netflix
switches state, it is still unstable. When Netflix increases utilization in the
on-off regime, Teams's utilization drops. As a result, not only does Teams
operate at extremely low utilization for nearly 75 seconds, it does not 
return to stable utilization until after then Netflix stream ends.}



Not only does 
Teams operate at extremely low utilization for nearly 75~seconds, 
its utilization is unstable afterwards.