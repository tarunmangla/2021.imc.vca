...
provide desktop applications as well as browser clients, whereas Meet only has
browser clients. Most of our tests are conducted using the desktop
...
our experimental setup for a 2-party call.  The two client the call are C1 and
...
(\texttt{tc}). A pre-recorded talking-head video\footnote{Using the device
webcam feed is a non-starter because without movement, VCAs compress the video
and ultimately send at a much lower rate than during a normal call.} with a
resolution of 1280 $\times$ 720 is used as the video source for the call. This
is done to both replicate a real video call and ensure consistency across
