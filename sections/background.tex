...
While choice of relay servers can significantly impact end-user experience, especially for geographically distributed clients~\cite{jiang2016via}, we believe the
paths to the relay servers are likely not the bottleneck in our case for two reasons: i) all of our participants are located within the University network with well-provisioned access link and upstream transit connections, and ii) the experiments are conducted in Chicago, which is in a large metro area. 
As a result, it is likely that there is well-provisioned infrastructure that supports Teams, Meet, and Zoom and provide good service.}
